...
from actuaryai.models.market import Market, Outcome
from actuaryai.models.risk import RiskAnalysis
...
class MarketSearch:
    ...
    def search(self, risk_analysis: RiskAnalysis, use_cache_only: bool = True) -> list[Market]:
        ...
        if not use_cache_only:
            ...
            if not cached:
                logger.error("No cached markets found. Run 'actuaryai update-cache' first.")
                raise ValueError("Market cache is empty. Please run 'actuaryai update-cache' to fetch markets first.")
